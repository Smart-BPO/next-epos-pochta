  ...
  return (
    <div className="space-y-6">
      <div>
        <DashBreadcrumbs
          items={[
            { href: "/dashboard/settings/", label: tg.breadcrumbsSettings },
            { label: tg.title },
          ]}
        />
        <h1 className={cn(dashPageTitle, "mt-1")}>{tg.title}</h1>
        <p className={cn(dashPageLead, "max-w-2xl")}>{tg.lead}</p>
      </div>

      <section className={cn(dashCardPad, "max-w-3xl")}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className={dashSectionTitle}>{tg.statusTitle}</h2>
          <StatusPill
            ok={webhookActive}
            okLabel={tg.statusActive}
            badLabel={tg.statusInactive}
          />
        </div>
      ...
      <section className={cn(dashCardPad, "max-w-3xl")}>
        <h2 className={dashSectionTitle}>{tg.commandsTitle}</h2>
