...
- Env:
 ...
 - FCargo status sync: inbound `POST /api/fcargo/webhook/` (header `X-Fcargo-Webhook-Secret`) + pull `POST /api/fcargo/sync/` (same secret or `FCARGO_SYNC_SECRET`). Calculator leads stay in `epos_fcargo_orders`; **all** webhook events are cataloged in `epos_fcargo_packages` (any channel). Mini App login links packages by phone → `epos_webapp_shipments` for Track. Request traffic in `epos_fcargo_request_log`.
