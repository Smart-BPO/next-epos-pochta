...
- Env:
 ...
 - FCargo status sync: inbound `POST /api/fcargo/webhook/` (header `X-Fcargo-Webhook-Secret`) + pull `POST /api/fcargo/sync/` (same secret or `FCARGO_SYNC_SECRET`). Orders linked in `epos_fcargo_orders`; all Client API / webhook traffic logged in `epos_fcargo_request_log`.
