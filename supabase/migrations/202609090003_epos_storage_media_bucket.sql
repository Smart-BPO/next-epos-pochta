...
drop policy if exists epos_media_public_read on storage.objects;
create policy epos_media_public_read
  on storage.objects for select to anon, authenticated
  using (bucket_id = 'epos-media');

drop policy if exists epos_media_staff_insert on storage.objects;
create policy epos_media_staff_insert
  on storage.objects for insert to authenticated
  with check (bucket_id = 'epos-media' and private.is_epos_admin());

drop policy if exists epos_media_staff_update on storage.objects;
create policy epos_media_staff_update
  on storage.objects for update to authenticated
  using (bucket_id = 'epos-media' and private.is_epos_admin())
  with check (bucket_id = 'epos-media' and private.is_epos_admin());
